...
class CurrentUserRoleView(APIView):
    """API view to GET/PATCH current authenticated user's profile.
    Supports multipart image upload (field name: 'avatar') which will be
    uploaded to Cloudinary and the returned secure_url saved on the profile.avatar field.
    """
    ...
    def patch(self, request):
        ...
        if avatar_file:
            try:
                ...
                return Response({'detail': 'Failed to upload avatar'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        # Update profile fields if provided
        reg_no = request.data.get('registration_number')
        dept = request.data.get('department')
        if reg_no is not None:
            profile.registration_number = reg_no or None
        if dept is not None:
        ...
        try:
            ...
            logger.exception('Failed to save profile/user')
